...
def build_rich_pitcher_prop(side_key: str, pitcher_data: dict, prop_event: dict,
    ...
    # Pull throws hand and recent starts from the game payload's pitcher_profile
    # (not on the projection dict itself).
    prof = ((game_payload or {}).get(side_key) or {}).get("pitcher_profile") or {}
...
<!-- Force browsers to revalidate so daily updates are picked up immediately -->
...
.pb-last5 { background: rgba(5,9,19,0.4); border: 1px solid var(--border); border-radius: 12px;
            padding: 12px; margin: 12px 0; }
.pb-last5 .l { font-size: 9px; font-weight: 800; color: var(--muted); letter-spacing: 0.10em;
               text-transform: uppercase; margin-bottom: 8px; }
.pb-last5 table { width: 100%; border-collapse: collapse; font-family: 'JetBrains Mono', monospace; font-size: 11px; }
.pb-last5 th { text-align: right; padding: 4px 6px; color: var(--muted); font-weight: 700;
               font-size: 9px; letter-spacing: 0.05em; text-transform: uppercase;
               border-bottom: 1px solid var(--border); }
.pb-last5 th:first-child { text-align: left; }
.pb-last5 td { text-align: right; padding: 5px 6px; color: var(--fg); font-weight: 600;
               border-bottom: 1px solid rgba(255,255,255,0.04); }
.pb-last5 td:first-child { text-align: left; color: var(--muted); }
.pb-last5 td.hi { color: var(--green); }
.pb-last5 td.lo { color: var(--red); }
.pb-last5 .avg td { background: rgba(255,255,255,0.04); font-weight: 800; color: var(--fg); }
.pb-last5 .avg td:first-child { color: var(--fg); }
...
// Cache-bust every JSON fetch with the page-load timestamp so the browser
// never serves a stale snapshot. The manifest is the first thing we fetch
// (no cache busting needed — fresh on every page load via meta no-cache),
// then everything else uses the manifest's generated_at as a stable version
// token. Bypass any intermediate cache layers explicitly.
const PAGE_LOAD_TS = Date.now();
async function fetchJSON(url) {
  const sep = url.includes('?') ? '&' : '?';
  const v = (manifest && manifest.generated_at) || PAGE_LOAD_TS;
  const r = await fetch(`${url}${sep}v=${encodeURIComponent(v)}`, {cache: 'no-store'});
...
async function loadManifest() {
  ...
  $('#live-label').textContent = `🟢 ${built} UTC`;
  $('#live-label').setAttribute('title', 'Last server-side rebuild (UTC). If you see stale data, hard-refresh (Cmd/Ctrl+Shift+R).');
  $('#footer').textContent = `Updated daily by the GitHub Actions cron · last build: ${built} UTC · client loaded at ${new Date().toISOString().replace('T',' ').slice(0,16)}`;
...
function noPlayRowHTML(gm) {
  const time = fmtGameTimeET(gm.gameDate);
  const am = n => (n === null || n === undefined) ? '—' : (n > 0 ? '+' + n : '' + n);
...
function teamCardHTML(side, label, gm) {
  ...
  let propBlock = '';
  const propHdr = (extra) => `<div style="font-size:10px;font-weight:700;color:var(--muted);text-transform:uppercase;letter-spacing:0.06em;margin-bottom:6px;">Starter prop projection${extra||''}</div>`;
  if (pk || pbb) {
    ...
    propBlock = `<div style="margin-top:10px;padding-top:8px;border-top:1px solid var(--border);">
      ${propHdr(`${starts!=null?` · ${starts}-start window`:''}${dq && dq!=='ok'?` · ${dq} data`:''}`)}
    ...
    // No projection for this starter — render a consistent placeholder so the
    // section appears on every game. Reason depends on what's missing.
    const reason = !t.probable_pitcher_name ? 'starter not yet announced'
                 : (prof.available === false) ? 'insufficient Statcast history (needs 3+ recent starts)'
                 : 'no projection available yet';
    propBlock = `<div style="margin-top:10px;padding-top:8px;border-top:1px solid var(--border);">
      ${propHdr()}
      <div class="muted" style="font-size:12px;">No projection — ${reason}.</div>
    </div>`;
  }
...
function propBoardHTML(p) {
  ...
  // Last 5 starts table — highlights the stat that matches this prop's market
  const starts5 = (p.last_5_starts || []);
  const propKey = (p.market_key === 'pitcher_walks') ? 'bb' : 'k';
  ...
  if (starts5.length) {
    ...
    const rows = starts5.map(s => {
      ...
      return `<tr>
        ...
        <td>${s.ip != null ? s.ip.toFixed(1) : '—'}</td>
        <td>${s.bf ?? '—'}</td>
        <td>${s.pitches ?? '—'}</td>
    ...
    const avg = (key) => {
      const vals = starts5.map(s => s[key]).filter(v => v != null);
      return vals.length ? (vals.reduce((a,b)=>a+b,0) / vals.length) : null;
    };
    const avgK = avg('k'), avgBB = avg('bb'), avgPitches = avg('pitches'), avgBF = avg('bf'), avgIP = avg('ip'), avgCSW = avg('csw'), avgVelo = avg('velo');
    const avgRow = `<tr class="avg">
      <td>AVG (${n})</td>
      <td>${avgIP != null ? avgIP.toFixed(1) : '—'}</td>
      <td>${avgBF != null ? avgBF.toFixed(1) : '—'}</td>
      <td>${avgPitches != null ? avgPitches.toFixed(0) : '—'}</td>
      <td>${avgK != null ? avgK.toFixed(1) : '—'}</td>
      <td>${avgBB != null ? avgBB.toFixed(1) : '—'}</td>
      <td>${avgCSW != null ? avgCSW.toFixed(0)+'%' : '—'}</td>
